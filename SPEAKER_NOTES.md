...
### [25] Zero-copy interop with the Arrow ecosystem · XIAO · ~0:35
One interop win worth calling out: the PyCapsule protocol. Spark now speaks the Arrow C Data Interface. So you hand data to Polars, DuckDB, and other Arrow-native tools with no copy — when both sides support it. Less glue and less overhead between Spark and the rest of your Python stack.
...
### [53] CDC implementation: Spark owns the hard parts · DB · ~0:40
This is the short version of Gengliang and Johan's CDC implementation talk. Spark 4.2 makes CDC first-class in the engine: one CHANGES syntax, one algorithm, and a small DSv2 connector contract. The connector declares the output columns, provides a scan builder for the raw change rows, names row identity and row version, and exposes capability flags such as whether the raw stream contains carry-over rows, intermediate changes, or updates represented as delete-plus-insert. Spark owns the post-processing: carry-over removal, update detection into preimage and postimage rows, and optional net-change collapse. Delta is the first implementation path here: row tracking gives Spark _metadata.row_id and _metadata.row_commit_version, so changes can be computed at read time without stored change files. For the full CDC implementation deep dive, see Gengliang Wang and Johan Lasperas's talk, "First-Class CDC Support in Spark 4.2," on June 18 from 10:20 to 11:00 at Marriott Level 2 Foothill G.
