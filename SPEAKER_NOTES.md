...
(XIAO:) Today we will show you what is new in Apache Spark 4.2. I will present the first half. DB will present the second half. The theme this year is: native analytics for data and AI. The common thread is that Spark is bringing more of the analytics stack into the engine. Let's start.
...
Benefit one: define truth once. Let's start with the problem, in plain words. And the takeaway: move your top dashboard metrics into a metric view.

### [7] The same metric should not change when the query changes · XIAO · ~0:35
Start with the pain, in plain words. Revenue per user is a ratio. And ratios are not additive — you cannot average them across groups and get the right answer. So when every team writes the formula by hand, in its own tool, the dashboards drift apart. Same metric, different numbers. Let me show you exactly how it breaks.

### [8] Some metrics don’t add up · XIAO · ~0:45
Here it is, in one table. Two countries. Revenue and active users add up fine — 200 and 15. But revenue per user is a ratio. Average the two rows and you get 15. That is wrong. The correct answer is total revenue over total users — 200 over 15 — which is 13.33. So the real question is: where should this metric be defined, so it comes out right every time? That is a semantic-layer problem.

### [9] Why this is a semantic-layer problem · XIAO · ~1:05
...
### [10] Metric views: define metrics once · XIAO · ~1:05
...
### [11] Metric views in action · XIAO · ~0:40
Three ideas, not syntax. A dimension is how people slice the metric. A measure is what the business cares about — here, revenue per user, a ratio. And the query is how analysts consume it: ask by any breakdown. The important point is not the syntax. The important point is that the metric is now an object Spark understands — so it is computed correctly at every grain, for everyone. Now — how do you connect to Spark?
...
### [17] Arrow on by default · XIAO · ~0:40
...
### [18] Python UDFs: same code, now faster · XIAO · ~0:25
...
### [21] Python Data Sources: connectors in pure Python · XIAO · ~0:50
...
### [22] A growing Python Data Source ecosystem · XIAO · ~0:30
...
### [25] Better interop & developer experience · XIAO · ~0:40
...
### [26] PyCapsule interop in action · XIAO · ~0:30
...
### [27] Debug UDFs where they run · XIAO · ~0:45
...
### [28] UDF debugging in action · XIAO · ~0:25
...
### [30] Query embeddings where your data lives · XIAO · ~0:45
...
### [33] NEAREST BY in action · XIAO · ~0:25
(Code slide.) One query: for each row, give me the five nearest. That is it. No window. No cross join.

### [34] SQL gets more natural · XIAO · ~0:45
Before and now, four times — each one removes a pattern many of us have written for years. QUALIFY: top-N without a subquery. FILTER: conditional aggregates now run over a window too — SUM(...) FILTER (WHERE ...) OVER (...), which was rejected before 4.2 (SPARK-55702). PIVOT with aliases: readable pivot columns. And cursors: row-by-row logic, now in pure SQL — no need to leave SQL for a loop. Less boilerplate, more intent. Takeaway: reach for QUALIFY first, and use FILTER over windows where you used to fall back to a subquery.
...
### [36] Data sketches: approximate, mergeable analytics · XIAO · ~0:55
...
### [37] Tuple sketches in action · XIAO · ~0:30
...
### [38] Native geospatial types · XIAO · ~0:45
...
### [39] Geospatial in action · XIAO · ~0:30
...
### [40] Section divider — Declarative Pipelines & Auto CDC · DB · ~0:15
...
### [41] Applying a change feed is the hard part · DB · ~0:50
...
### [45] Evolve running pipelines safely · DB · ~0:40
...
### [48] A state store you can trust · DB · ~0:35
...
### [49] Section divider — Data Source V2 · DB · ~0:15
...
### [50] One integration API for every data source · DB · ~0:40
...
### [51] Ask the table: "what changed?" · DB · ~0:45
...
### [52] Two problems with CDC today · DB · ~0:50
...
### [53] One CHANGES API for changelog-capable DSv2 sources · DB · ~0:45
...
### [56] Smarter writes: INSERT & MERGE · DB · ~0:40
...
### [59] Observable, evolvable DML · DB · ~0:35
...
### [60] Smarter pushdown: PartitionPredicate · DB · ~0:35
...
### [62] A modern Spark Web UI · DB · ~0:30
...
### [65] Section divider — Looking Ahead · DB · ~0:15
...
### [67] Project Feather: fast local queries · DB · ~0:45
...
### [68] Project Feather: a three-part plan · DB · ~0:55
...
### [72] Nanosecond-precision timestamps · DB · ~0:40
...
### [73] Real-time mode for stateful streaming · DB · ~0:40
...
### [74] Faster, predictable releases · DB · ~0:50
